...
class CompetenceService {
    ...
    public function updateCompetence($request, $id){
        ...
        for ($i = 0; $i < count($newGroupe); $i++) {
            $grpComp = $this->repoGroupeCom->findBy(array('libelle' => $newGroupe[$i]["libelle"]));
            if (!is_null($grpComp)) {
                $competence->addGroupeCompetence($grpComp[0]);
            }else {
                return new JsonResponse("Le groupe de compétence ¦¦".$newGroupe[$i]["libelle"]."¦¦ n'existe pas.", Response::HTTP_BAD_REQUEST, [], true);
            }
        }
