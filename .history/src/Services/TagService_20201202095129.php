...
class TagService
{
    ...
    /**Archivage */
        ...
        $tagsBrute = $data['tags'];
        $toAdded = $this->updateLink($tagsBrute , $groupeTag);
    ...
    public function updateLink($tagsBrute,$groupe){
        //Removed
        foreach ($tagsBrute as $key) {
            $tags[] = $key["libelle"];
        }
        foreach ($groupe->getTags() as $tagDB) {

            if (!in_array($tagDB->getLibelle(), $tags)) {
                //dd("in");
                $groupe->removeTag($tagDB);
            }
        }

    //Added
        foreach ($groupe->getTags() as  $value) {
