...
class GroupeCompetenceService
{
    ...
    //Removed
        foreach ($competencesBrutes as $key) {
            $competences[] = $key["libelle"];
        }
        foreach ($groupeCompetence->getCompetences() as $tagDB) {
    ...
    //Added
        ...
        foreach ($competences as $newLibelle) {
            if (!in_array($newLibelle, $allCompetences)) {
                $competenceAdded = $this->repoTag->findBy(array('libelle' => $newLibelle));
                if ($competenceAdded) {
        ...
        //$this->manager->persist($groupeCompetence);
        //$this->manager->flush();
        $GroupeCompetenceJson = $this->serializer->serialize($groupeCompetence, 'json',["groups"=>["promo_groupe_apprenants:read"]]);
        return new JsonResponse($GroupeCompetenceJson, Response::HTTP_OK, [], true);
