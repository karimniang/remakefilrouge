...
//Modifier un referentiel
    public function updateReferentiel($request, $id)
    {
       ...
       // $referentielUpdate = $this->serializer->serialize($referentiel, 'json');
        ...
        $groupeCompetenceBrutes = $data['groupeCompetences'];
        ...
        /*foreach ($data['groupeCompetences'] as $value) {
            if ($value != "") {
                ...
                if (empty(explode("/", $value)[1])) {
                    return new JsonResponse("Veuillez choisir une action pour le grp de comp =>.".$libelle, Response::HTTP_BAD_REQUEST, [], true);
                }
