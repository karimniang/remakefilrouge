...
class GroupeCompetenceController extends AbstractController
{
    ...
    public function addGroupeCompetence(GroupeCompetenceService $groupeCompetenceService, Request $request){
     ...
     /** 
     * @Route("/admin/grpecompetences/{id}/{action}", name="put_groupe_competence", methods ="PUT")
     */
    public function UpdateGroupeCompetence(GroupeCompetenceService $groupeCompetenceService, Request $request,$id, $action){
        if ($this->isGranted('ROLE_APPRENANT')) {
            return new JsonResponse("Vous n'êtes pas autorisé à modifier un groupe de compétences.", Response::HTTP_BAD_REQUEST, [], true);
        }
        return $groupeCompetenceService->updateGroupeCompetence($request, $id, $action);
    }
}
