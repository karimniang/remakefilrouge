...
class ProfilSortieVoter extends Voter
{
    ...
    protected function voteOnAttribute($attribute, $subject, TokenInterface $token)
    {
        ...
        switch ($attribute) {
            ...
            break;
            case 'PROFIL_VIEW':
                if ($user->getRoles()[0] === "ROLE_FORMATEUR" || $this->security->isGranted('ROLE_ADMIN')) {
                    return true;
                }
            break;
            case 'PROFIL_ALL_VIEW':
                if ($this->security->isGranted('ROLE_ADMIN') || $this->security->isGranted('ROLE_CM') || $this->security->isGranted('ROLE_FORMATEUR')) {
                    return true;
                }
            break;
            case 'PROFIL_POST':
